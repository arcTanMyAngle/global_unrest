...
aggregate chatter, and the opt-in Daily Events digest path. Fixtures remain
deterministic tests and the worker/service smoke input; the desktop does not
load them.

Daily Events is Anthropic-backed, not Gemini-backed. It is a separate page,
not a source: the user selects a UTC day and explicitly requests generation.
The result is cached locally and has two required sections, media attention
and event data.

## Non-negotiable product rules

1. Keep media attention, discrete events, official alerts, aggregate chatter,
   and generated prose separate. Never present a combined result as truth.
...
6. Daily Events must remain opt-in, bounded, labelled, and two-sectioned.
   ...
   request path.
7. Do not make the API public over ACLED-bearing worker aggregates. The API
...
The CI workflow runs each source feature separately, the full union, both
credentialed mock suites, Compose smoke coverage, and the workspace gates.
UI-visible changes need a live desktop run when feasible. A real
cargo build of the desktop is useful after dependency/linking changes because
check and clippy do not perform the final link.
...
- The Digest worker calls Anthropic only after an explicit request and never
  opens storage itself.
- The desktop deletes legacy fixture rows on startup and treats an empty
  live database as valid.
...
|---|---|
| core-types | Pure domain types and source contracts. |
...
| chatter, source-bluesky, source-telegram | Aggregate-only social chatter pipeline. |
| analytics | Bucket scoring, baselines, spikes, divergence. |
| storage | DuckDB actor, migrations, queries, exports, snapshots. |
| daily-digest | Bounded day facts, schema, parsing, optional Anthropic transport. |
| renderer | Cached egui layers, glyphs, alerts, graticule, labels. |
| global-signal-desktop | Desktop application/UI. |
...
- docs/DATA_MODEL.md for types, migrations, and storage.
- docs/ARCHITECTURE.md for runtime/process ownership.
- docs/API.md for routes or middleware.
- docs/SAFETY_AND_PRIVACY.md for source terms, privacy, or third-party
  processing.
