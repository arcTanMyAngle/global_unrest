...
| LES_TELEGRAM_CHANNEL_CATALOG | Optional path to a TOML classified channel catalog (`[[channel]]` rows with mandatory `id`, `handle`, `class`, `region`, `cadence_secs`). When set it replaces the compiled-in neutral allowlist for both ingest and Media search. A file that fails validation — a missing class/region, a malformed handle, a duplicate id/handle — is a startup error, never a partial load. See docs/adr/0001-ondemand-media-sources.md. |
